...
NGINX_CONF_URI = os.getenv('NGINX_CONF_URI',
...
handler.setFormatter(
    logging.Formatter(
        '%(asctime)s - MMIB - %(name)s - %(levelname)s - %(message)s'
    )
)
...
def reload_nginx(client, api_gateways):
    ...
    nginx_containers = client.containers.list(filters={'label': NGINX_LABEL,
                                                       'status': 'running'})
...
def render_template(ips):
    """
    This function renders the template of nginx configuration file
    and write it to mounted volume.
...
def get_currently_running_web_servers(client):
    web_containers = client.containers.list(filters={'label': WORKER_LABEL,
                                                     'status': 'running'})
